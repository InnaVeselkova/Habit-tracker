...
class HabitTests(TestCase):
    ...
    def test_create_habit_with_linked_non_pleasure(self):
        ...
        self.assertIn(
            "В связанной привычке должна быть признак 'приятной привычки'.", response.data["linked_habit"][0]
        )
    ...
    def test_filter_public_habits_only(self):
        ...
        Habit.objects.create(
            owner=self.user, place="Public Place", time="10:00", action="Read", is_public=True, time_spent_minutes=3
        )
        Habit.objects.create(
            owner=self.user,
            place="Private Place",
